...
def count_non_int_paths(face_list, start_edge, outer_boundary, cont_sections):
    ...
    for i in range(2, len(cont_sections)+1):  # Range is off because negative values are offset
        ...
        print("Working on section {0} with length {1}, expected memory {2}".format(len(cont_sections)-i, '.'.join([str(len(sect)) for sect in cur_sect]), math.prod([sum([1/(d2+1) * (math.comb(2*d2, d2) * (math.comb(len(sect), 2*d2) if 2*d2 < len(sect) else 1)) for d2 in range(depth_bound+1)]) for sect in cur_sect])))
        ...
        prev_dict = cur_dict
        prev_sect = cur_sect
        with open("Run_dicts/section{0}.txt".format(str(len(cont_sections) - i)), 'w') as outfile:
            first_few = list(cur_dict.keys())[:5000]
            outfile.write(str(dict({k: cur_dict[k] for k in first_few})))
...
def order_faces(graph, positions):
    ...
    while len(cur_boundary) > 1:
        ...
        for i in range(len(inds) - 1):
            if inds[i + 1] - inds[i] > 1:  # Bad face! Go to next? edge
                cur_edge_index = (cur_edge_index + 7) % (len(cur_boundary) - 1) if pass_counter < len(outer_face)*20 else (cur_edge_index + 1) % (len(cur_boundary) - 1)
                cont = True
    ...
    print("Depth bound: " + str(depth_bound))
    # exit(0)
    # print(face_list)
    # print(cont_sections)
    # table, edge_dict = allocate_table(face_list, start_edge, start_boundary_list, cont_sections)
    print("Finished setup: " + str(time.time()))
    count = count_non_int_paths_unrestr(face_list, start_edge, start_boundary_list, cont_sections)
...
if __name__ == '__main__':
    enumerate_paths("data/exp2627neighb.dbf", "data/exp2627wards.shp", draw=False)
    # test()
    exit(0)
